...
export default function CreateLobbyScreen() {
      ...
      setUserCards((prevUserCards) =>
        prevUserCards.map((card) =>
          card.name === payload.name
            ? { ...card, points: payload.points, karma: card.karma }
            : card
        )
      );
    };
  ...
  function enableTrap(trapName: string) {
    const updatedItems = shopItems.map((item) =>
      item.name === trapName ? { ...item, cooldown: false } : item
    );
  ...
  function disableTrap(trapName: string) {
    const updatedItems = shopItems.map((item) =>
      item.name === trapName ? { ...item, cooldown: true } : item
    );
    setShopItems(updatedItems);
                  ...
                  >
                    ...
                    <View style={styles.pointsContainerRight}>
                      ...
                      <View style={styles.pointsRow}>
                        ...
                        />
                        <Text style={styles.pointsText}>
                          {user.name === userName ? user.karma || "0" : "?"}
                        </Text>
                      </View>
                    </View>
                  </View>
            ...
            <View style={styles.shopGrid}>
              {shopItems.map((item) => (
                <TouchableOpacity
                  key={item.id}
                  disabled={
                    !gameStarted || karmaActual < item.cost || item.cooldown
                  }
                  style={[
                    styles.shopItem,
                    { backgroundColor: item.backgroundColor },
                    (!gameStarted ||
                      karmaActual < item.cost ||
                      item.cooldown) &&
                      styles.disabledItem,
                  ]}
  ...
  const puntosActuales =
    userCards.find((XDuwu) => XDuwu.name === userName)?.points ?? 0;
  // Chicos esto es para conservar el karma de el jugador y poder mantener el estado de los botones
  const karmaActual =
    userCards.find((XDuwu) => XDuwu.name === userName)?.karma ?? 0;
  useEffect(() => {
    setShopItems((AntStats) =>
      AntStats.map((stat) => ({
        ...stat,
        state: gameStarted && karmaActual >= stat.cost ? "enable" : "disable",
      }))
    );
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    width: "100%",
    paddingVertical: isSmallDevice ? 10 : 18,
